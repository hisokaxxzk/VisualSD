#include "VD.h"
std::vector<coordinates> dl_linkedlist(1000);
void Game::enter_DLL()
{
    int number_value = 0;
    int count_word = 0;
    ...
    printText(19, color["Grey"][0], color["Grey"][1], color["Grey"][2], "Enter the value of each node in the Doubly linked list, separated them by Enter.", 450, 20, 0, 0);
    while (true)
    {
        SDL_Event e;
        SDL_PollEvent(&e);
                                ...
                                for (int i = 0; i < number_value; i++)
                                {
                                    for (; pos <= text_find.length(); pos++)
                                    {
                                        if (text_find[pos] == ',')
                                        {
                                         ...
                                         dl_linkedlist[i + 1].nameID += text_find[pos];

                                    }

                                }
                                number_node = number_value;
                                render_dll( dl_linkedlist);
                                while (true)
                                {
                                    handleEvents_DLL();
                                }
                                        ...
                                        while (true)
                                        {
                                            ...
                                            for (; pos <= text_find.length(); pos++)
                                            {
                                                if (text_find[pos] == ',')
                                                {
                                                 ...
                                                 dl_linkedlist[m].nameID += text_find[pos];
                                            }
