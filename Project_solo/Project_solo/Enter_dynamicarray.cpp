#include "VD.h"
static std::vector<coordinates> dynamicarray(1000);
void Game::enter_data_dynamic_array()
{
    int number_value = 0;
    int count_word = 0;
    bool generate = false;
        ...
        if (e.type == SDL_MOUSEBUTTONDOWN)
        {
            if (mouseX >= 450 && mouseX <= 1050 && mouseY >= 50 && mouseY <= 85) // Enter node
            {
                ...
                while (true)
                {
                    ...
                    if (evt.type == SDL_MOUSEBUTTONDOWN) //Generate the array and go to the next page
                    {
                        SDL_GetMouseState(&mouseX, &mouseY);
                            ...
                            if (generate)
                            {
                                ...
                                for (int i = 0; i < number_value; i++)
                                {
                                    for (; pos <= text_find.length(); pos++)
                                    {
                                        ...
                                        dynamicarray[i + 1].nameID += text_find[pos];

                                    }

                                }
                                number_node = number_value;
                                render_dynamic_array(dynamicarray);
                                while (true)
                                {
                                    handleEvents_dynamic_array();
                                }
                            }
                        }
                        else
                                    ...
                                    else
                                    {
                                        ...
                                        while (true)
                                        {
                                            ...
                                            for (; pos <= text_find.length(); pos++)
                                            {
                                                ...
                                                dynamicarray[m].nameID += text_find[pos];
                                            }
