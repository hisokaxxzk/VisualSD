...
static bool head = false, tail1 = false, menu_linkedlist = false, show_menu_add = false, txtinp = false, btn_map = false, cancel_menu = false, show_menu_add_del;
    ...
    case SDL_MOUSEBUTTONDOWN:
        ...
        for (int i = 1; i <= number_coorbtn + number_node; i++)
        {
                    ...
                    mouseY >= linked_list[i].y_begin && mouseY <= linked_list[i].y_end))))
            {
                ...
                if (state_btn[i].nameID == "Enqueue") //Push data
                {
                    check_1st_time.menu_add = false;
                    show_menu_add = true;
                    makeRectangle(202, state_btn_map["Enqueue"].y_begin, 150, 32, "Vertex = ", 19, "Green", false, false, true);
                    //create input box
                    makeRectangle(272, state_btn_map["Enqueue"].y_begin + 5, 40, 25, "input_box", 19, "Black", false, false, false);
                    save_state_btn_map(272, state_btn_map["Enqueue"].y_begin + 5, 40, 25, "input_box");
                    makeLine(272 + 2, state_btn_map["Enqueue"].y_begin + 5 + 2, 272 + 2, state_btn_map["Enqueue"].y_begin + 7 + 20, "White");
                        ...
                        if (e.type == SDL_TEXTINPUT) {
                            if (inputText.length() <= 3)
                                if (isdigit(e.text.text[0])) {
                                    ...
                                    printText(15, 255, 255, 255, inputText, 272 + 2, state_btn_map["Enqueue"].y_begin + 5 + 2, 0, 0);
                                    e.type = 0;
                                }
                        }
...
void Game::render_queue(std::vector<coordinates> linkedlist) {
...
void Game::queue(std::string value, int current_node)
{
    ...
    if (!menu_linkedlist)
    {
        ...
        number_coorbtn++;
        drawRec(50, state_btn[number_coorbtn - 1].y_end + 2, 150, 38, true, "Enqueue", 20, "Green");
        save_state_btn(50, state_btn[number_coorbtn - 1].y_end + 2, 150, 38, "Enqueue");
        save_state_btn_map(50, state_btn[number_coorbtn - 1].y_end + 2, 150, 38, "Enqueue");
        number_coorbtn++;
        drawRec(50, state_btn[number_coorbtn - 1].y_end + 2, 150, 38, true, "Dequeue", 20, "Green");
        save_state_btn(50, state_btn[number_coorbtn - 1].y_end + 2, 150, 38, "Dequeue");
        save_state_btn_map(50, state_btn[number_coorbtn - 1].y_end + 2, 150, 38, "Dequeue");
        number_coorbtn++;
